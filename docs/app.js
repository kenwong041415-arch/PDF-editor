  ...
  for (let i = 0; i < opList.fnArray.length; i++) {
    ...
    if (fn === OPS.save) {
      ...
      for (let k = 0; k < coords.length; k += 2) {
        minX = Math.min(minX, coords[k]); maxX = Math.max(maxX, coords[k]);
        minY = Math.min(minY, coords[k + 1]); maxY = Math.max(maxY, coords[k + 1]);
      }
      const [px0, py0] = [ctm[0] * minX + ctm[2] * minY + ctm[4], ctm[1] * minX + ctm[3] * minY + ctm[5]];
      const [px1, py1] = [ctm[0] * maxX + ctm[2] * maxY + ctm[4], ctm[1] * maxX + ctm[3] * maxY + ctm[5]];
      const x0 = Math.min(px0, px1), x1 = Math.max(px0, px1);
      const y0 = Math.min(py0, py1), y1 = Math.max(py0, py1);
      // A ruled line: thin one way, long the other. Only the thin-and-wide
      // (horizontal) case is handled, matching what these forms actually use.
      if (x1 - x0 > 3 && y1 - y0 < 3) {
        lines.push({ x0, x1, y0, y1, color: fillColor });
      }
    }
  }
