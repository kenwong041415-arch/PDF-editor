 ...
 *     isn't shrunk to fit based on a false "too wide" reading. A ruled line
 *     just below the baseline (e.g. a blank-field underline) the cover
 *     rectangle would otherwise erase is found and redrawn at its exact
 *     geometry -- read from pdf.js's own parsed drawing operators
 *     (getOperatorList()), the same source of truth PyMuPDF's get_drawings()
 *     gives the Python engine, rather than guessed from rendered pixels.
 *
...
// --- Ruled-line detection (for restoring underlines a cover rectangle
// would otherwise erase) -------------------------------------------------
//
// A redaction-style cover rectangle wide enough to blank out text descenders
// often also erases any ruled line sitting just below the baseline (e.g. a
// signature/date blank drawn under a label). Three attempts at detecting
// these from the *rendered canvas* (a dark-pixel-ratio scan, then a
// longest-dark-run scan with wider padding) kept failing in different ways
// in practice: sub-pixel/anti-aliasing noise, a line only partially
// overlapping the edited text's own box, etc.
//
// pdf.js has to parse the page's actual drawing operators to render it at
// all, and exposes that parse via getOperatorList() -- so instead of
// re-deriving the line's geometry from pixels, this reads it from the same
// source PyMuPDF's get_drawings() uses on the Python side: the real filled
// rectangles in the PDF's content stream, with exact coordinates.
const ruledLineCache = new Map(); // pdf.js Page -> line rects (PDF space)
...
// Parse a page's operator list once into a flat list of thin filled
// rectangles ({x0, x1, y0, y1, color}) in raw PDF (bottom-left-origin)
// space -- the same space item.transform values already use elsewhere in
// this file. Tracks the graphics-state stack (save/restore), the CTM
// (transform), and the current fill colour, since a rectangle's on-page
// position and colour both depend on the state active when it was drawn.
async function getRuledLines(pdfPage) {
  if (ruledLineCache.has(pdfPage)) return ruledLineCache.get(pdfPage);
  const opList = await pdfPage.getOperatorList();
  const OPS = pdfjsLib.OPS;
  const FILL_OPS = new Set([OPS.fill, OPS.eoFill, OPS.fillStroke, OPS.eoFillStroke]);
  // How many (x, y) points each path-construction op consumes from the
  // flat coords array, needed to split a compound path back into the
  // individual subpaths it was built from (see below).
  const POINT_COUNT = {
    [OPS.moveTo]: 1, [OPS.lineTo]: 1,
    [OPS.curveTo]: 3, [OPS.curveTo2]: 2, [OPS.curveTo3]: 2,
    [OPS.closePath]: 0,
  };
  ...
  const stack = [];
  const lines = [];
  const transformPt = (x, y) => [ctm[0] * x + ctm[2] * y + ctm[4], ctm[1] * x + ctm[3] * y + ctm[5]];
  ...
  for (let i = 0; i < opList.fnArray.length; i++) {
    ...
    if (fn === OPS.save) {
      ...
      if (prev) { ctm = prev.ctm; fillColor = prev.fillColor; }
    ...
    } else if (fn === OPS.constructPath && FILL_OPS.has(opList.fnArray[i + 1])) {
      // A PDF commonly bundles several unrelated rectangles into one filled
      // path for file-size reasons -- confirmed on a real document: a 30pt
      // signature-blank underline fused into the same fill as a completely
      // unrelated 600pt rule elsewhere on the page. An aggregate bounding
      // box over the whole compound path is neither thin nor near either
      // real shape, so it would both miss the real line and risk treating a
      // huge, unrelated region as one. This walks subOps/coords and splits
      // the path back into its individual subpaths (each starting at a
      // moveTo), computing each one's own bounding box separately.
      ...
      const flushSubpath = () => {
        if (minX <= maxX) {
          const [px0, py0] = transformPt(minX, minY);
          const [px1, py1] = transformPt(maxX, maxY);
          const x0 = Math.min(px0, px1), x1 = Math.max(px0, px1);
          const y0 = Math.min(py0, py1), y1 = Math.max(py0, py1);
          // A ruled line: thin one way, long the other -- only the
          // thin-and-wide (horizontal) case is handled, matching what
          // these forms actually use.
          if (x1 - x0 > 3 && y1 - y0 < 3) lines.push({ x0, x1, y0, y1, color: fillColor });
        }
        minX = Infinity; minY = Infinity; maxX = -Infinity; maxY = -Infinity;
      };

      for (const op of subOps) {
        if (op === OPS.moveTo) flushSubpath();
        const npts = POINT_COUNT[op];
        if (npts === undefined) { malformed = true; break; } // unhandled op (e.g. a curve-heavy shape) -- skip this path rather than guess
        for (let p = 0; p < npts; p++) {
          const x = coords[coordIdx], y = coords[coordIdx + 1];
          coordIdx += 2;
          minX = Math.min(minX, x); maxX = Math.max(maxX, x);
          minY = Math.min(minY, y); maxY = Math.max(maxY, y);
        }
      }
      if (!malformed) flushSubpath();
    }
  }
  ruledLineCache.set(pdfPage, lines);
  return lines;
}

// Ruled lines overlapping a text run's horizontal span, within the vertical
// zone a cover rectangle reaches to blank out descenders (where such a line
// would otherwise get erased). x0/x1/yBaseline/fontSize are in PDF space.
function findRuledLines(lines, x0, x1, yBaseline, fontSize) {
  const top = yBaseline + fontSize * 0.94;
  const bottom = yBaseline - fontSize * 0.26;
  return lines.filter((l) => l.x1 > x0 && l.x0 < x1 && l.y1 > bottom && l.y0 < top);
}
...
function openEditor(rec, box) {
  ...
  const commit = async () => {
    ...
    await recordEdit(rec, box, newText);
    buildOverlay();
    updateEditCount();
  };
  const cancel = () => {
...
async function recordEdit(rec, box, newText) {
  const key = `${state.page}:${rec.index}`;
  ...
  const lines = await getRuledLines(state.pdfPage);
  const ruledLines = findRuledLines(lines, t[4], t[4] + rec.item.width, t[5], fontSize);
  state.edits.set(key, {
    page: state.page,
    x: t[4],
    yBaseline: t[5],
...
async function replaceAll() {
  ...
  try {
    ...
    for (let p = 0; p < state.pageCount; p++) {
      ...
      const lines = await getRuledLines(page);
      for (const rec of matches) {
        ...
        state.edits.set(`${p}:${rec.index}`, {
          ...
          ruledLines: findRuledLines(lines, t[4], t[4] + rec.item.width, t[5], fontSize),
        });
        count++;
      }
    }
  ...
  try {
    ...
    for (const edit of state.edits.values()) {
      ...
      try {
        ...
        // 3. Restore any ruled line (e.g. a blank-field underline) that
        // step 1 covered over, at its exact original geometry.
        for (const rl of edit.ruledLines || []) {
          page.drawRectangle({
            x: rl.x0,
            y: rl.y0,
            width: rl.x1 - rl.x0,
